...
{
    public partial class NPLInvestmentDetailsAdmin : System.Web.UI.Page
    {
        ...
        List<string> ignoreForUploadCheck = new List<string>
    {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    if (Session["sCompID"].ToString() != null)
                    {
                        ...
                        else
                        {
                            ...
                            ViewState["EmpCode"] = (string)Session["sEmpID"];
        ...
        protected void btnLoadDetails_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //if (txtEmpMid.Text.Trim() != "")
                    ...
                    {
                    //DataSet ds = objInv.CheckEmp((string)Session["sCompID"], txtEmpMid.Text.Trim());
                    DataSet ds = objInv.CheckEmp((string)Session["sCompID"], drpEmpType.SelectedValue.Trim());
                    if (ds.Tables.Count > 0)
                    {
                        if (ds.Tables[0].Rows.Count > 0)
                        {
                            ViewState["EmpCode"] = ds.Tables[0].Rows[0][0].ToString();
        ...
        private void FillAll()
        {
            ...
            DataSet dsRegime = objInv.GetRegime((string)Session["sCompID"], (string)ViewState["EmpCode"]);
        ...
        {
            dsInv = objInv.GetInvestmentDetails((string)Session["sCompID"], (string)ViewState["EmpCode"]);
        ...
        private void Fill_Amounts()
        {
            ...
            objInv.GetAmountDetails((string)Session["sCompID"], (string)ViewState["EmpCode"], lblInvDetails, lnkRent, lnkRentNew, lnk12, lnk12b);
        }

        private void FillOther()
        {
            dsInv = objInv.GetOtherDetails((string)Session["sCompID"], (string)ViewState["EmpCode"]);
        ...
        {
            dsInv = objInv.GetRentDetails((string)Session["sCompID"], (string)ViewState["EmpCode"], txtAddress);
            gvRent.DataSource = dsInv;
            gvRent.DataBind();
            dsInv = new DataSet();
            dsInv = objInv.GetLandlordDetails((string)Session["sCompID"], (string)ViewState["EmpCode"]);
        ...
        {
            dsInv = objInv.GetTwelveDetails((string)Session["sCompID"], (string)ViewState["EmpCode"]);
            gvtwelve.DataSource = dsInv;
            gvtwelve.DataBind();
            dsInv = new DataSet();
            dsInv = objInv.GetLenderDetails((string)Session["sCompID"], (string)ViewState["EmpCode"]);
        ...
        {
            dsInv = objInv.GetTwelveBDetails((string)Session["sCompID"], (string)ViewState["EmpCode"]);
        ...
        protected void lnkUpdate_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                result = objInv.UpdateInvestment(MakeInvXml(gvInv, ""), "", "", (string)Session["sCompID"], (string)ViewState["EmpCode"]);
        ...
        private Boolean CheckPrevAmount()
        {
            ...
            foreach (GridViewRow gvr in this.gvTwelB.Rows)
            {
                if (((Label)gvr.FindControl("lblId")).Text == "AD000010")
                {
                    amount = Convert.ToDouble((((TextBox)gvr.FindControl("txtAmt")).Text == "" ? "0" : ((TextBox)gvr.FindControl("txtAmt")).Text));
                    break;
                }
        ...
        private string MakeInvXml(GridView GV, string strType)
        {
            ...
            foreach (GridViewRow gvr in GV.Rows)
            {
                ...
                sbTaxDetails.Append(" EMP_AID='" + (string)ViewState["EmpCode"] + "'");
        ...
        protected void lnkUpdateRent_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                result = objInv.UpdateInvestment(MakeInvXml(gvRent, "Rent"), txtAddress.Text.Trim(), "Rent", (string)Session["sCompID"], (string)ViewState["EmpCode"]);
                if (result.ToString().Trim() == "success")
                {
                    result = objInv.UpdateInvestment(MakeInvXml(gvLandlordDetails, "Landlord"), "", "", (string)Session["sCompID"], (string)ViewState["EmpCode"]);
                    if (result.ToString().Trim() == "success")
                    {
                        FillRents();
                        CalculateInvestments(gvRent, "RENT");
        ...
        protected void lnkUpdate12_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                result = objInv.UpdateInvestment(MakeInvXml(gvtwelve, "12"), "", "", (string)Session["sCompID"], (string)ViewState["EmpCode"]);
                if (result.ToString().Trim() == "success")
                {
                    result = objInv.UpdateInvestment(MakeInvXml(gvLenderDetails, "Lender"), "", "", (string)Session["sCompID"], (string)ViewState["EmpCode"]);
                    if (result.ToString().Trim() == "success")
                    {
                        FillTwelve();
                        CalculateInvestments(gvtwelve, "12C");
        ...
        protected void gvInv_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                dspass = objInv.Getpassword((string)Session["sCompID"], (string)ViewState["EmpCode"], lblMID.Text);
        ...
        protected void lnkUpdate12B_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                result = objInv.UpdateInvestment(MakeInvXml(gvTwelB, ""), "", "", (string)Session["sCompID"], (string)ViewState["EmpCode"]);
                if (result.ToString().Trim() == "success")
                {
                    FillTwelveB();
                    CalculateInvestments(gvTwelB, "12B");
        ...
        protected void lnkUpdateOther_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                result = objInv.UpdateInvestment(MakeInvXml(gvOther, "Other"), "", "", (string)Session["sCompID"], (string)ViewState["EmpCode"]);
                if (result.ToString().Trim() == "success")
                {
                    divAlertSucc.Visible = true;
                    lblMessageSucc.Visible = true;
        ...
        protected void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                DsTest = objBl.Fill_Report((string)(ViewState["EmpCode"]), (string)(Session["sCompID"]));
        ...
        protected void btnUpload_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                fStream.Close();
                bWriter.Close();
                objSup.Upload_Support_Password((string)Session["sCompID"], (string)ViewState["EmpCode"], lblMID.Text.Trim() + "_" + fileName, "2021-2022", txtPassword.Text.Trim());
                //DisplayDocuments();
        ...
        private void UploadFile12BB()
        {
            try
            {
                if (System.IO.Path.GetExtension(fup12BB.FileName).ToUpper() == ".PDF" || System.IO.Path.GetExtension(fup12BB.PostedFile.FileName).ToUpper() == ".ZIP" || System.IO.Path.GetExtension(fup12BB.PostedFile.FileName).ToUpper() == ".RAR" || System.IO.Path.GetExtension(fup12BB.PostedFile.FileName).ToUpper() == ".7Z" || System.IO.Path.GetExtension(fup12BB.PostedFile.FileName).ToUpper() == ".JPG" || System.IO.Path.GetExtension(fup12BB.PostedFile.FileName).ToUpper() == ".JPEG")
                {
                    ...
                    objInv.Upload_12BB((string)Session["sCompID"], (string)ViewState["EmpCode"], Convert.ToString(Session["sEmpCode"]) + ext);
                    SendSuccessMail();
        ...
        private void SendSuccessMail()
        {
            if (((string)Session["sEmailId"]).Trim() != "")
            {
                ...
                mailMessage.Subject = "Acknowledgement for Investment for F.Y. 2022-2023";
                mailMessage.Body = "Dear Sir/Madam,\n\nForm 12BB for 2022-2023 has been received.\n\nWith Best Regards,\nPayrollservices";
        ...
        private void UploadFileRent()
        {
            try
            {
                ...
                fStream.Close();
                bWriter.Close();
                objSup.Upload_Support_Password((string)Session["sCompID"], (string)ViewState["EmpCode"], "RENT_" + fileName, "2022-2023", txtPasswordRent.Text.Trim());
                //DisplayDocuments();
        ...
        private void UploadFileLoan()
        {
            try
            {
                ...
                fStream.Close();
                bWriter.Close();
                objSup.Upload_Support_Password((string)Session["sCompID"], (string)ViewState["EmpCode"], "LOAN_" + fileName, "2022-2023", txtPasswordLoan.Text.Trim());
                //DisplayDocuments();
        ...
        private void UploadFile12B()
        {
            try
            {
                ...
                fStream.Close();
                bWriter.Close();
                objSup.Upload_Support_Password((string)Session["sCompID"], (string)ViewState["EmpCode"], "12B_" + fileName, "2022-2023", txtPassword12B.Text.Trim());
                //DisplayDocuments();
        ...
        private void UploadUndertaking()
        {
            try
            {
                ...
                fStream.Close();
                bWriter.Close();
                objSup.Upload_Support_Password((string)Session["sCompID"], (string)ViewState["EmpCode"], "UNDER_" + fileName, "2021-2022", "");
                //DisplayDocuments();
        ...
        private void SubmitAll()
        {
            ...
            try
            {
                ...
                result = objInv.UpdateInvestmentAll(other, inv, rent, landlord, twelve, lender, prev,
                            txtAddress.Text.Trim(), "Rent", (string)Session["sCompID"], (string)ViewState["EmpCode"]);
                // result = "success";
                ...
                if (result.ToString().Trim() == "success")
                {
                    try
                    {
                        ...
                        //    string fullpath = fileList[0];
                        //    string[] arypath = fullpath.Split('\\');

                        //        string folderName = arypath[10];

                        //        if (folderName == "FLEXI")
                        //        {
                        //            string folderSourcePaths = "";
                        //            if ((string)Session["sCompID"] == "CO000114")
                        //            {
                        //                folderSourcePaths = Request.PhysicalApplicationPath + Convert.ToString(ConfigurationManager.AppSettings.Get("Path")) + Convert.ToString(Session["sCompID"]) + "\\ANGEL\\Documents\\" + Convert.ToString(Session["sEmpCode"]).ToUpper() + "\\FLEXI\\";
                        //            }
                        //            else
                        //            {
                        //                folderSourcePaths = Request.PhysicalApplicationPath + Convert.ToString(ConfigurationManager.AppSettings.Get("Path")) + Convert.ToString(Session["sCompID"]) + "\\" + Convert.ToString(Session["sCompAID"]) + "\\Documents\\" + Convert.ToString(Session["sEmpCode"]).ToUpper() + "\\FLEXI\\";
                        //            }
                        //            if (Directory.Exists(folderSourcePaths))
                        //            {
                        //                string[] folderfileList = System.IO.Directory.GetFileSystemEntries(folderSourcePaths);

                        //                FileUpload folderfupUpload = new FileUpload();
                        //                foreach (string folderfile in folderfileList)
                        //                {
                        //                    string[] foldersplitpath = folderfile.Split('\\');
                        //                    //string path1 = splitpath[8];
                        //                    string folderpath = foldersplitpath[11];
                        //                    string[] foldernewPaths = folderpath.Split('.');
                        //                    string folderFilePath = foldernewPaths[0];
                        //                    string folderext = foldernewPaths[1];
                        //                    //string nameOnly=FilePath.Split('-').Last();
                        ...
                        //                    byte[] byteArray = Encoding.UTF8.GetBytes(folderFilePath);

                        //                    MemoryStream stream = new MemoryStream(byteArray);

                        //                    System.IO.Stream fileInputStream = stream;
                        //                    Byte[] fileContent = new Byte[fileInputStream.Length];
                        //                    fileInputStream.Read(fileContent, 0, Convert.ToInt32(fileInputStream.Length));
                        //                    fileInputStream.Close();
                        //                    //
                        //                    if (System.IO.Directory.Exists(@savePath) == false)
                        //                    {
                        //                        File.SetAttributes(folderfile, FileAttributes.Normal);
                        //                        File.Delete(folderfile);
                        //                        System.IO.Directory.CreateDirectory(@savePath);
                        //                    }

                        //                    DateTime indianTime = TimeZoneInfo.ConvertTime(DateTime.Now, TimeZoneInfo.FindSystemTimeZoneById("India Standard Time"));
                        //                    string fileName = Path.GetFileNameWithoutExtension(folderfupUpload.FileName.Trim().Replace(" ", "_")) + "_"
                        //                        + indianTime.ToString("ddMMyyyyHHmmss") + Path.GetExtension(folderfupUpload.FileName.Trim());

                        //                    System.IO.FileStream fStream = new System.IO.FileStream(@savePath + folderFilePath + fileName + "." + folderext, System.IO.FileMode.Create);
                        //                    System.IO.BinaryWriter bWriter = new System.IO.BinaryWriter(fStream);
                        //                    bWriter.Write(fileContent);
                        //                    fStream.Flush();
                        //                    bWriter.Flush();
                        ...
                        //                    string[] foldersplitpath = folderfile.Split('\\');
                        //                    //string path1 = splitpath[8];
                        //                    string filename = foldersplitpath[11];
                        //                    File.Delete(Path.Combine(folderSourcePaths, filename));
                        ...
                        //if ((string)Session["sCompID"] == "CO000114")
                        //{
                        //    fileSourcePath = Request.PhysicalApplicationPath + Convert.ToString(ConfigurationManager.AppSettings.Get("Path")) + Convert.ToString(Session["sCompID"]) + "\\ANGEL\\Documents\\" + Convert.ToString(Session["sEmpCode"]).ToUpper();
                        //}
                        //else
                        //{
                        //    fileSourcePath = Request.PhysicalApplicationPath + Convert.ToString(ConfigurationManager.AppSettings.Get("Path")) + Convert.ToString(Session["sCompID"]) + "\\" + Convert.ToString(Session["sCompAID"]) + "\\Documents\\" + Convert.ToString(Session["sEmpCode"]).ToUpper();
                        //}
                        ...
                        //        string[] fileLists = System.IO.Directory.GetFileSystemEntries(fileSourcePath);
                        //        FileUpload fupUpload = new FileUpload();
                        //        foreach (string files in fileLists)
                        //        {
                        //            string[] splitpath = files.Split('\\');

                        //            string path = splitpath[10];
                        //            if (path != "FLEXI")
                        //            {
                        //                string[] newPaths = path.Split('.');
                        //                string FilePath = newPaths[0];
                        //                string ext = newPaths[1];
                        ...
                        //                System.IO.FileStream fStream = new System.IO.FileStream(@savePath + FilePath + fileName + "." + ext, System.IO.FileMode.Create);
                        //                System.IO.BinaryWriter bWriter = new System.IO.BinaryWriter(fStream);
                        //                bWriter.Write(fileContent);
                        //                fStream.Flush();
                        //                bWriter.Flush();
                        ...
                        //        foreach (string files in fileLists)
                        //        {
                        //            string[] foldersplitpath = files.Split('\\');
                        //            //string path1 = splitpath[8];

                        //            string filename = foldersplitpath[10];
                        //            if (filename != "FLEXI")
                        //            {
                        //                File.Delete(Path.Combine(fileSourcePath, filename));
                        //            }
                        //        }
                        //    }
                        ...
                        if ((string)Session["sCompID"] == "CO000056")
                        {
                            ...
                            {
                                objFlexi.UpdateFlexipaySupport("AD000159", (string)Session["sCompID"], (string)ViewState["EmpCode"], txtLTAAmt.Text.Trim(), txtLTANo.Text.Trim());
                            }

                            if (Convert.ToDouble(txtTelAmt.Text.Trim() == "" ? "0" : txtTelAmt.Text.Trim()) != 0)
                            {
                                objFlexi.UpdateFlexipaySupport("AD000207", (string)Session["sCompID"], (string)ViewState["EmpCode"], txtTelAmt.Text.Trim());
                            }

                            if (Convert.ToDouble(txtFuelAmt.Text.Trim() == "" ? "0" : txtFuelAmt.Text.Trim()) != 0)
                            {
                                objFlexi.UpdateFlexipaySupport("AD000150", (string)Session["sCompID"], (string)ViewState["EmpCode"], txtFuelAmt.Text.Trim());
                            }

                            if (Convert.ToDouble(txtDriverAmt.Text.Trim() == "" ? "0" : txtDriverAmt.Text.Trim()) != 0)
                            {
                                objFlexi.UpdateFlexipaySupport("AD000153", (string)Session["sCompID"], (string)ViewState["EmpCode"], txtDriverAmt.Text.Trim());
                            }
                        }

                        if (result.ToString().Trim() == "success")
                        {
                            if ((string)Session["sCompID"] == "CO000056" || (string)Session["sCompID"] == "CO000126")
                            {
        ...
        private void GenerateAndEmailForm12BB()
        {
            try
            {
                ...
                if ((string)Session["sCompID"] == "CO000056")
                {
                    ReportDataSource datasource = new ReportDataSource("dsEmp_dtEmpInd", DsTest.Tables[0]);
                    rptPrint.LocalReport.DataSources.Clear();
                    rptPrint.LocalReport.ReportPath = @"ESS\ReportIndiafirstWithReimb.rdlc";
                    rptPrint.LocalReport.DisplayName = (string)(Session["sEmpCode"]) + "_Form12BB";
                    rptPrint.LocalReport.DataSources.Add(datasource);
                    rptPrint.LocalReport.SubreportProcessing += new SubreportProcessingEventHandler(LocalReport_SubreportProcessing);
                    //rptPrint.LocalReport.Refresh();
                    ...
                    if (((string)Session["sEmailId"]).Trim() != "")
                    {
                        ...
                        if (System.IO.File.Exists(Request.PhysicalApplicationPath.ToString() + "PDF Reports\\" + Convert.ToString(Session["sCompID"]) + "\\" + Convert.ToString(Session["sCompAID"]) + "\\Form12BB\\" + (string)(Session["sEmpCode"]) + "_Form12BB.pdf"))
                        {
                            ...
                            mailMessage.To.Add(((string)Session["sEmailId"]).Trim());
                            mailMessage.From = new MailAddress("reports@sequelgroup.co.in");
                            mailMessage.Subject = "Acknowledgement for Investment/Flexi Proofs - FY 2022 - 2023";
                            mailMessage.Body = "Dear Sir/Madam,\n\n This mail is towards acknowledgement of Investments / Flexi proofs uploaded by you in Investment & Flexi portal.\n\n"
                ...
                else if ((string)Session["sCompID"] == "CO000126")
                {
                    ...
                    if (((string)Session["sEmailId"]).Trim() != "")
                    {
                        if (System.IO.File.Exists(Request.PhysicalApplicationPath.ToString() + "PDF Reports\\" + Convert.ToString(Session["sCompID"]) + "\\" + Convert.ToString(Session["sCompAID"]) + "\\Form12BB\\" + (string)(Session["sEmpCode"]) + "_Form12BB.pdf"))
                        {
                            ...
                            mailMessage.To.Add(((string)Session["sEmailId"]).Trim());
                            mailMessage.From = new MailAddress("reports@sequelgroup.co.in");
                            mailMessage.Subject = "Acknowledgement for Investment - FY 2021 - 2022";
                ...
                else
                {
                    ...
                    if (((string)Session["sEmailId"]).Trim() != "")
                    {
                        ...
                        mailMessage.To.Add(((string)Session["sEmailId"]).Trim());
                        mailMessage.From = new MailAddress("reports@sequelgroup.co.in");
                        mailMessage.Subject = "Acknowledgement for Investments for F.Y. 2021-2022";
                        mailMessage.Body = "Dear Sir/Madam,\n\n Investments received for 2021-2022.\n\nWith Best Regards,\nPayrollservices";
        ...
        private void UploadFileLTA()
        {
            try
            {
                ...
                objSup.Upload_Support_Flexi((string)Session["sCompID"], (string)ViewState["EmpCode"], "LTA_" + fileName, "2022-2023");
                //DisplayDocuments();
        ...
        private void UploadFileTel()
        {
            try
            {
                ...
                objSup.Upload_Support_Flexi((string)Session["sCompID"], (string)ViewState["EmpCode"], "TEL_" + fileName, "2022-2023");
                //DisplayDocuments();
        ...
        private void UploadFileFuel()
        {
            try
            {
                ...
                objSup.Upload_Support_Flexi((string)Session["sCompID"], (string)ViewState["EmpCode"], "FUEL_" + fileName, "2022-2023");
                //DisplayDocuments();
        ...
        private void UploadFileDriver()
        {
            try
            {
                ...
                objSup.Upload_Support_Flexi((string)Session["sCompID"], (string)ViewState["EmpCode"], "DRV_" + fileName, "2022-2023");
                //DisplayDocuments();
        ...
        private void FillSupportDetails()
        {
            try
            {
                objFlexi.GetSupportDetails((string)Session["sCompID"], (string)ViewState["EmpCode"], txtLTANo, txtLTAAmt, txtTelAmt, txtFuelAmt, txtDriverAmt,
