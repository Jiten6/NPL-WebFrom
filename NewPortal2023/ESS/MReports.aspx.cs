...
{
    public partial class MReports : System.Web.UI.Page
    {
        ...
        private void FillMonths()
        {
            ...
            else if (Session["sCompID"].ToString() == "CO000141")
            {
                System.IO.DirectoryInfo dirInfo = new DirectoryInfo(Request.PhysicalApplicationPath.ToString() + Convert.ToString(ConfigurationManager.AppSettings.Get("Path")) + Convert.ToString(Session["sCompID"]) + "\\" + Convert.ToString(Session["sCompAID"]) + "\\MONTHLYREPORTS\\");
                System.IO.DirectoryInfo[] fileNames = dirInfo.GetDirectories("*.*");
                sbDetails.Append("<ROOT>");
                foreach (DirectoryInfo dir in fileNames)
                {
        ...
        private void DisplayDocuments()
        {
            try
            {
                ...
                SourcePath = Convert.ToString(ConfigurationManager.AppSettings.Get("Path")) + Convert.ToString(Session["sCompID"]) + "\\" + Convert.ToString(Session["sCompAID"]) + "\\" + "\\MONTHLYREPORTS\\" + Convert.ToString(drpMonth.SelectedItem.Value.Trim());
